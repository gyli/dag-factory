...
from dagfactory import schema, settings
...
class DagBuilder:
    ...
    def get_dag_params(self) -> Dict[str, Any]:
        ...
        for callback_type in [
            ...
            # sla_miss_callback is gone as of Airflow 3.1. The schema records
            # that (x-airflow-max-version), so validate_config() reports it and
            # _build_dag_kwargs() drops it; there is nothing to resolve here.
    ...
    def validate_config(dag_params: Dict[str, Any]) -> None:
        """Check a resolved DAG config against the bundled JSON schema.

        Runs the same schema, through the same code path, that
        ``dagfactory lint`` uses, so a config that lints clean builds without
        complaint here. Findings are logged; under ``strict_mode`` an error
        stops the build rather than letting Airflow fail further down.
        """
        ...
        for issue in validator.iter_issues(dag_params, dag_id=dag_id):
            if issue.severity == "error":
                errors.append(issue)
                logger.error("Invalid DAG config: %s", issue.render())
            else:
                logger.warning("DAG config warning: %s", issue.render())

        if errors and settings.strict_mode:
            raise DagFactoryConfigException(
                f"DAG '{dag_id}' failed schema validation: " + "; ".join(i.render() for i in errors)
            )

    @staticmethod
    def _build_dag_kwargs(dag_params: Dict[str, Any]) -> Dict[str, Any]:
        """Builds the kwargs passed to the DAG constructor, driven by the JSON schema.

        The schema decides which keys are DAG arguments and which Airflow
        ...
        arguments = schema.dag_argument_names()
        ...
        for key in dag_params:
            ...
            reason = schema.unsupported_reason(key, INSTALLED_AIRFLOW_VERSION)
            if reason:
                warnings.warn(reason, UserWarning)
                continue

            value = dag_params[key]
            transform = schema.TRANSFORMS.get(key)
            if transform:
        ...
        for key, target, value in aliased:
            warnings.warn(
                f"`{key}` param is deprecated. Please use {target}.",
