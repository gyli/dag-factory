"""Tests for dagfactory.validator: metadata-driven DagParameterValidator."""
...
def test_validate_yaml_content_catches_schema_error():
    ...
    rendered = " ".join(i.render() for i in results[0].errors)
    assert "catchup" in rendered and "bool" in rendered
...
def test_x_dagfactory_supported_false_is_a_warning_not_error():
    """supported: False (template_undefined) is a warning — the value is valid
    Airflow but silently ignored by dag-factory at runtime."""
    yaml_text = (
        "my_dag:\n"
    ...
    results = DagParameterValidator(airflow_version="3").validate_yaml_content(yaml_text)
    assert not results[0].errors
    assert any("not wired through dag-factory" in w.message for w in results[0].warnings)
...
def test_x_airflow_min_version_is_an_error():
    """min_version (allowed_run_types, introduced in 3.2) errors when the
    configured Airflow predates it."""
    yaml_text = (
        "my_dag:\n"
